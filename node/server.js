...
const { community } = require("./steam-bot")
...
let displayName = config.defaultProfile.display
let avatar = config.defaultProfile.icon;

let defaultName = config.defaultProfile.display;
let defaultAvatar = config.defaultProfile.icon;
    ...
    if (typeof data.map !== "undefined") {
        ...
        if (data.map.name !== general_data.map || data.map.mode !== general_data.game_mode) {
            ...
            for (const [key, value] of Object.entries(config.map)) {
                if (config.map[key].id === data.map.name) {
                    console.log(`[INFO] match found at ${data.map.name}!`)
                    console.log(`[INFO] updating display to ${displayName}`)
                    displayName = config.map[key].display
                    avatar = config.map[key].icon
...
function processMapEnd(data) {
    if (data.previously.map.phase === "live" && data.map.phase === "gameover") {
        ...
        // Set our user details back to default
        editProfileName(defaultName)
        editProfileAvatar(defaultAvatar)
...
function processBombStatus(data) {
    if (data.round.bomb === "planted") {
        ...
        if (config.bombPlanted == true) {
            console.log("[INFO] Setting to bombPlanted config");
            editProfileName(config.bombPlanted.display);
        }
